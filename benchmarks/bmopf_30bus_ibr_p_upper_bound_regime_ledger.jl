...
"""Record bound regimes and strict complementarity outcomes for 30-bus IBR rows."""
...
function regime(bound, zero_tolerance)
    abs(bound) <= zero_tolerance ? "zero_bound" : "positive_bound"
end

function run_case(root, relative, max_iter, zero_tolerance, complementarity_tolerance)
    ...
    JuMP.optimize!(model)
    point = NLPDiagnostics.solver_result_point(model; label="30bus-bound-regime-result")
    ...
    for row in target_rows
        ...
        gap = Float64(bound) - value
        push!(rows, Dict{String,Any}(
            "row" => row,
            "regime" => regime(Float64(bound), zero_tolerance),
            "bound" => Float64(bound),
            "value" => value,
            "model_gap" => gap,
            "physical_gap" => gap * scale,
            "scale" => scale,
            "complementarity_residual" => get(side, "complementarity_residual", nothing),
            "strict_complementarity_passed" => get(side, "complementarity_residual", Inf) <= complementarity_tolerance,
        ))
    end
    summary = Dict{String,Any}()
    for current_regime in ("zero_bound", "positive_bound")
        members = filter(row -> row["regime"] == current_regime, rows)
        isempty(members) && continue
        finite = field -> Float64[Float64(row[field]) for row in members if row[field] isa Real]
        residuals = finite("complementarity_residual")
        summary[current_regime] = Dict{String,Any}(
            "row_count" => length(members),
            "bound_range" => begin values = finite("bound"); [minimum(values), maximum(values)] end,
            "model_gap_range" => begin values = finite("model_gap"); [minimum(values), maximum(values)] end,
            "physical_gap_range" => begin values = finite("physical_gap"); [minimum(values), maximum(values)] end,
            "complementarity_residual_range" => [minimum(residuals), maximum(residuals)],
            "strict_complementarity_passed_count" => count(row -> row["strict_complementarity_passed"], members),
        )
    end
    return Dict{String,Any}(
        ...
        "rows" => rows,
        "qualification" => "bound-regime and strict-complementarity ledger; not a causal explanation",
    )
end

root = abspath(get(ENV, "NLPDIAGNOSTICS_BMOPF_BENCHMARK_ROOT", DEFAULT_ROOT))
isdir(root) || error("benchmark root does not exist: $root")
max_iter = parse(Int, get(ENV, "NLPDIAGNOSTICS_BMOPF_BOUND_REGIME_MAX_ITER", "80"))
zero_tolerance = parse(Float64, get(ENV, "NLPDIAGNOSTICS_BMOPF_BOUND_REGIME_ZERO_TOLERANCE", "1e-12"))
complementarity_tolerance = parse(Float64, get(ENV, "NLPDIAGNOSTICS_BMOPF_BOUND_REGIME_COMPLEMENTARITY_TOLERANCE", "1e-5"))
max_iter > 0 || error("max_iter must be positive")
zero_tolerance >= 0 || error("zero tolerance must be nonnegative")
complementarity_tolerance >= 0 || error("complementarity tolerance must be nonnegative")
results = Dict{String,Any}[]
for relative in selected_cases(root)
    try
        push!(results, run_case(root, relative, max_iter, zero_tolerance, complementarity_tolerance))
    catch error
        push!(results, Dict{String,Any}("snapshot" => relative, "error" => sprint(showerror, error)))
    end
end
output = abspath(get(ENV, "NLPDIAGNOSTICS_BMOPF_BOUND_REGIME_OUTPUT", joinpath(@__DIR__, "..", "work", "bmopf-30bus-ibr-p-upper-bound-regime-ledger.json")))
mkpath(dirname(output))
write_json(output, Dict(
    "schema_version" => "nlpdiagnostics-bmopf-30bus-ibr-p-upper-bound-regime-ledger-v1",
    "source" => Dict(
...
println("wrote 30-bus IBR upper bound-regime ledger to $output")
