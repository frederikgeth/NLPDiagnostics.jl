...
"""Run a bounded initialization matrix for the 30-bus IBR upper row."""
...
const POLICIES = ("none", "bmopf", "bmopf_zero_completion", "zero")
...
function apply_initialization!(context, policy)
    ...
    policy == "none" && return Dict{String,Any}(
        ...
        "existing_finite_start_count" => existing_count,
        "finite_start_count" => existing_count,
        "missing_start_count" => length(variables) - existing_count,
        "provenance_kind" => "EngineInitialization",
    )
    point = policy == "bmopf" ?
        NLPDiagnostics.bmopf_initialization_point(context) :
        policy == "bmopf_zero_completion" ?
        NLPDiagnostics.bmopf_start_completion_point(
            context; missing_value=0.0, label="30bus-zero-completed-start",
        ) : nothing
    if policy == "bmopf" && !(point isa NLPDiagnostics.EvaluationPoint)
        return Dict{String,Any}(
            ...
            "finite_start_count" => existing_count,
            "missing_start_count" => length(variables) - existing_count,
            "provenance_kind" => "UnavailableInitializationPoint",
        )
    end
    finite_count = 0
    missing_count = 0
    for (ordinal, variable) in enumerate(variables)
        value = policy == "zero" ? 0.0 : point.values[ordinal]
    ...
    return Dict{String,Any}(
        ...
        "coordinate_count" => length(variables),
        "existing_finite_start_count" => existing_count,
        "finite_start_count" => finite_count,
        "missing_start_count" => missing_count,
        "provenance_kind" => policy == "zero" ? "InitializationPoint" :
            policy == "bmopf_zero_completion" ? "CompletedInitializationPoint" :
            "BMOPFInitializationPoint",
        "point_provenance_complete" => point isa NLPDiagnostics.EvaluationPoint ?
            point.provenance.complete : true,
    )
end
...
function run_case(root, relative, policy, max_iter)
    ...
    initialization = apply_initialization!(context, policy)
    model = BMOPFTools.opf_model(context)
    JuMP.set_silent(model)
    JuMP.set_optimizer_attribute(model, "max_iter", max_iter)
    JuMP.optimize!(model)
    point = NLPDiagnostics.solver_result_point(model; label="30bus-initialization-result")
...
max_iter = parse(Int, get(ENV, "NLPDIAGNOSTICS_BMOPF_30BUS_INITIALIZATION_MAX_ITER", "80"))
max_iter > 0 || error("max_iter must be positive")
results = Dict{String,Any}[]
for relative in selected_cases(root)
    for policy in POLICIES
        try
            push!(results, run_case(root, relative, policy, max_iter))
        catch error
...
output = abspath(get(
    ENV,
    "NLPDIAGNOSTICS_BMOPF_30BUS_INITIALIZATION_OUTPUT",
    joinpath(@__DIR__, "..", "work", "bmopf-30bus-ibr-p-upper-initialization-matrix.json"),
))
mkpath(dirname(output))
write(output, JSON.json(Dict(
    "schema_version" => "nlpdiagnostics-bmopf-30bus-ibr-p-upper-initialization-matrix-v1",
    "source" => Dict(
...
println("wrote 30-bus IBR initialization matrix to $output")
