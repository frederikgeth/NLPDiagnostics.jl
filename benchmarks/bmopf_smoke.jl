...
function _benchmark_case(spec, context, point_policy::String)
    ...
    elseif point_policy == "zero"
        ...
        error("unknown NLPDIAGNOSTICS_BMOPF_POINT_POLICY='$point_policy' (use initialization or zero)")
    end
    return NLPDiagnostics.ProfileCase(spec.name, point;
        ...
        scale = "per-unit",
        tags = spec.tags,
        metadata = Dict("fixture" => spec.file, "point_policy" => point_policy),
    )
end
...
function main()
    ...
    for spec in fixtures
        ...
        try
            ...
            run = NLPDiagnostics.bmopf_build_and_profile(network,
                ...
                profile_kwargs = (include_initialization = true,),
            )
    ...
    write(joinpath(output_dir, "index.json"), JSON.json(Dict(
        "fixture_root" => abspath(root), "cases" => index,
    )))
