...
"""Repeat the static affine-row cache A/B on mixed-density and nonlinear workloads."""
...
const OUTPUT = abspath(isempty(ARGS) ?
    joinpath(ROOT, "docs", "analyze_static_optimization_generalization_summary.json") : ARGS[1])

function parse_positive_list(raw::AbstractString)
    values = try
        parse.(Int, split(raw, ','))
    catch
        error("NLPDIAGNOSTICS_ANALYZE_STATIC_GENERALIZATION_DIMENSIONS must be comma-separated integers")
    end
    isempty(values) && error("generalization dimensions must not be empty")
    all(>(2), values) || error("generalization dimensions must be greater than two")
    length(unique(values)) == length(values) || error("generalization dimensions must not contain duplicates")
    return values
end

dimensions = parse_positive_list(get(
    ENV, "NLPDIAGNOSTICS_ANALYZE_STATIC_GENERALIZATION_DIMENSIONS", "64,128,256",
))
repetitions = try
    parse(Int, get(ENV, "NLPDIAGNOSTICS_ANALYZE_STATIC_GENERALIZATION_REPETITIONS", "3"))
catch
    error("NLPDIAGNOSTICS_ANALYZE_STATIC_GENERALIZATION_REPETITIONS must be a positive integer")
end
repetitions > 0 || error("generalization repetitions must be positive")

function mixed_density_chain_model(dimension::Int)
    ...
    for index in 1:(dimension - 2)
        terms = MOI.ScalarAffineTerm{Float64}[
            ...
            MOI.ScalarAffineTerm(0.25, variables[index + 2]),
        ]
...
function nonlinear_chain_model(dimension::Int)
    model = MOI.Utilities.Model{Float64}()
    variables = [MOI.add_variable(model) for _ in 1:dimension]
    for index in 1:(dimension - 1)
        difference = MOI.ScalarNonlinearFunction(:-, Any[variables[index + 1], variables[index]])
...
function static_result(snapshot, graph, cache_affine_coefficients)
    GC.gc()
    ...
    return Dict{String,Any}(
        "stage" => get(report.metadata, :stage, "missing"),
...
function measure_workload(builder::Function, workload::AbstractString)
    ...
    for dimension in dimensions
        ...
        for repetition in 1:repetitions
            ...
            equivalent = baseline["stage"] == candidate["stage"] &&
                baseline["finding_count"] == candidate["finding_count"] &&
...
for builder in (mixed_density_chain_model, nonlinear_chain_model)
    model = builder(first(dimensions))
    snapshot = NLPDiagnostics.snapshot(model)
    graph = NLPDiagnostics.incidence_graph(snapshot)
    NLPDiagnostics.analyze_static(snapshot; graph = graph, cache_affine_coefficients = false)
    NLPDiagnostics.analyze_static(snapshot; graph = graph, cache_affine_coefficients = true)
end

records = vcat(
    measure_workload(mixed_density_chain_model, "mixed_density_affine_chain"),
    measure_workload(nonlinear_chain_model, "sparse_nonlinear_chain"),
)
status_entries = git_status_entries()
write_json(OUTPUT, Dict{String,Any}(
    "schema_version" => "nlpdiagnostics-analyze-static-optimization-generalization-v1",
    "source" => Dict(
        "runner" => "benchmarks/profile_analyze_static_optimization_generalization.jl",
        "dimensions" => dimensions,
        "repetitions" => repetitions,
        "workloads" => ["mixed_density_affine_chain", "sparse_nonlinear_chain"],
    ...
    "interpretation" => Dict(
        "claim" => "The cached affine-row candidate preserves static finding identities, stage metadata, and propagation metadata on mixed-density affine and sparse nonlinear fixtures while recording local timing and allocation deltas.",
        "does_not_establish" => [
...
println("wrote analyze_static optimization generalization summary to $OUTPUT")
