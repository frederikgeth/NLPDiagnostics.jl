...
const ACTIONS = Dict{String,Dict{String,Any}}(
    "numerical_rank_false_positive_negative_statistics" => Dict(
        "priority" => 1,
        "next_deliverable" => "Record the project-owner decision on the threshold-policy review handoff, then implement and revalidate the selected path.",
    ...
    "runtime_memory_scaling" => Dict(
        "priority" => 3,
        "next_deliverable" => "Carry the reviewed voltage-start API contract and proposal document upstream; pursue peak-capable allocator telemetry and a convergent hard-OPF larger-case experiment only under explicit guards.",
    ...
    "analyze_runtime_scaling" => Dict(
        "priority" => 4,
        "next_deliverable" => "Review the matched two-environment OS peak candidate and current-allocation stage telemetry, then obtain peak-capable allocator telemetry before making a portable runtime or memory claim.",
