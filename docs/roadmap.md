...
`benchmarks/compare_bmopf_formulation_interventions.jl` to
`benchmarks/common.jl`, bringing the shared helper to seventeen core runners
while preserving the comparison's CLI and report schema. Its local smoke run
completed successfully and retained the explicit non-causal readiness fields.
The campaign and evidence-ledger summarizers and comparisons, solver-trace and
summary comparisons, residual-trend, and checkout validators remain covered;
the checkout validator's contract stage passes, while its child-suite rerun is
...
gates now use the same JSON/repository helper.
