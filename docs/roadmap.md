...
`benchmarks/bmopf_30bus_ibr_p_upper_directional_jacobian_check.jl` to
`benchmarks/common.jl`, bringing the shared helper to sixty-nine core runners
while preserving the analytic-versus-finite-difference directional audit
schema. Its local smoke selected one 30-bus LN snapshot and completed without
an error, covering 28 target rows with two directional reports. The prior
increment migrated
...
sixty-nine core release, rank, runtime, and audit runners; the BMOPF campaign and
