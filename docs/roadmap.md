...
99-bus LN, 99-bus LG, and 538-bus LN solver-result points with complete
provenance and available sparse estimates; all three are scaling-stable (zero
...
with solver-result behavior. The next step is to add more 538-bus time
snapshots and retain endpoint-level span diagnostics.
