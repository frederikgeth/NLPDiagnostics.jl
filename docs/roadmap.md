...
`benchmarks/summarize_bmopf_multiconductor_smoke.jl` to `benchmarks/common.jl`,
bringing the shared helper to forty core runners while preserving the
multiconductor contract, integrity, source-schema, and readiness schema. Its
local smoke run completed successfully against the merged smoke-results index,
retaining two non-successful cases and their explicit readiness boundaries. The
...
forty core release, rank, runtime, and audit runners; the BMOPF campaign and
