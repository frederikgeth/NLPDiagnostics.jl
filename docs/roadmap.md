...
`benchmarks/compare_bmopf_solver_traces.jl` to `benchmarks/common.jl`, bringing
the shared helper to sixteen core runners while preserving the comparison's
CLI and report schema. Its local smoke run passed against the tracked trace
summary and retained the four case comparisons. The campaign and
evidence-ledger summarizers and comparisons, summary comparison, residual-trend,
and checkout validators remain covered; the checkout validator's contract stage
...
sixteen core release, rank, runtime, and audit runners; the BMOPF campaign and
evidence-ledger summarizers and comparisons, solver-trace and summary
comparisons, residual-trend, checkout, and PR-handoff gates now use the same
