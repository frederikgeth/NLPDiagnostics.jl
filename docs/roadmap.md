...
| Degeneracy | Strong | `dependent_row_localization` finds one bounded, point-local numerical dependent row set and verifies every one-row deletion under a fixed threshold. Synthetic cross-point and active-set controls, plus a 79-row checked-in OPF development case, are covered; independently prepared application calibration remains open. |
...
2. maintain and calibrate the delivered dependent-row localization, taking the
   checked-in OPF endpoint protocol to independently prepared applications;
...
## 2026-08-27 reviewed t15/t16 tranche

The fresh t15/t16 LN/LG SI/PU tranche was rerun through the isolated local
BMOPFTools environment with dense rank disabled and the explicit sparse guards.
All eight endpoints (four snapshots paired across SI and PU) have 11,028
variables and 12,538 rows, complete `SolverResultPoint` provenance, and full
11,028-coordinate saved-result mapping with zero fallback coordinates. Every
endpoint retains sparse-QR rank 11,028 under both unscaled and row-column
policies; the four SI/PU pairs have zero unit-dependent rank outcomes. The
review ledger is recorded in `docs/bmopf_t15_t16_tranche_summary.json` and is
generated by `benchmarks/summarize_bmopf_t15t16_tranche.jl`.

This tranche is practical evidence that the repaired saved-result path remains
stable across adjacent high-voltage snapshots and both fault families. It does
not close the broader numerical release gate: threshold-sensitive cross-backend
...
## 2026-08-27 threshold-policy review handoff

The policy choice is now represented by an explicit review handoff in
`docs/rank_threshold_policy_review_summary.json`, generated by
`benchmarks/review_rank_threshold_policy.jl`. The handoff verifies that the
classification evidence is internally consistent (49 hard controls, zero hard
mismatches, zero unavailable backends, and nine classified disagreements) and
offers two bounded options: accept the current policy boundary, or add and vet
an independent backend. It intentionally records `review_required` with no
default decision; this prevents a machine-generated artifact from silently
changing rank tolerances or promoting the experimental normal-eigen path.
...
handoff in `docs/real_99bus_kkt_boundary_review_summary.json`, generated by
`benchmarks/review_real_99bus_kkt_boundary.jl`. It confirms 2/6 strict paired
acceptances and 4/6 strict failures, with every failure localized to the
already-known `ibr_p_upper` family. The maximum required tolerance is
approximately 1.14172e-5 versus the unchanged strict 1e-5 gate; this is margin
evidence, not a relaxation recommendation. The handoff offers retaining the
strict gate or conducting a physically justified tolerance review, and records
`review_required` with no automatic decision.

The next deliverable is the project-owner disposition for this physical-KKT
boundary, followed by revalidation under the selected policy.

## 2026-08-27 solver-scaling extension plan

The runtime track now has an execution-ready plan in
`docs/bmopf_solver_scaling_extension_plan.json`, generated by
`benchmarks/plan_bmopf_solver_scaling_extension.jl`. It preserves the current
four measured solver records at 4,180/4,902 variables and the explicit
56,142-variable size guard, then selects two reviewed 11,028-variable 538-bus
Ipopt cases (LG and LN t15) for the next bounded child-process measurements.
The plan fixes a 12,000-variable guard, 180-second timeout, 50-iteration cap,
dense-rank disablement, sparse-QR fill guards, and endpoint/termination
acceptance criteria. It explicitly forbids fitting a complexity law from the
small tranche or presenting cumulative `Sys.maxrss` as allocator-level peak
memory.

The plan is ready for execution; the next deliverable is the guarded 538-bus
solver run and its termination, timing, and provenance summary.

## 2026-08-27 guarded 538-bus solver extension

The planned LG/LN t15 538-bus cases were executed through the isolated
BMOPFTools solver-trace harness with `max_iter=50`, `print_level=0`, a
12,000-variable solve guard, and a 180-second child timeout. Both 11,028-
variable Ipopt children completed without timeout and produced complete
callback-point traces: LG converged in 27 iterations and LN in 34. The
machine-readable result is `docs/bmopf_solver_scaling_extension_summary.json`,
generated by `benchmarks/summarize_bmopf_solver_scaling_extension.jl`.

The trace-only resource policy skipped post-solve profiling, so this closes a
solver-work/termination evidence increment but not allocator-level memory or
complexity claims. The next deliverable is to add an independent solver or a
larger guarded case before making a runtime-scaling claim.

## 2026-08-27 cross-solver 538-bus trace extension

The same LG/LN t15 snapshots were rerun with MadNLP under the identical
12,000-variable, 180-second child-process, and 50-iteration guards. Both
11,028-variable cases completed with complete callback traces: LG in 27
iterations and LN in 36. The paired ledger in
`docs/bmopf_solver_scaling_cross_solver_summary.json`, generated by
`benchmarks/summarize_bmopf_solver_scaling_cross_solver.jl`, records 2/2 paired
successes and zero incomplete processes; the iteration and residual deltas are
descriptive only and do not establish solver superiority or production
scalability. The trace-only policy still leaves allocator-level peak memory and
larger-ladder evidence open. The next deliverable is a larger guarded case or
allocator-level telemetry.

## 2026-08-27 full combined MV/LV bounded probe

The full BMOPFTools combined MV/LV source case was promoted from a size-only
guard to a bounded solver probe. Its 56,142-variable model reached Ipopt for
the classic, explicit MV/LV-local, and SI policies under a 60,000-variable,
one-iteration, five-second-per-policy budget. All three returned
`ITERATION_LIMIT` with one result and no solver error. The normalized evidence
is `docs/bmopf_combined_mv_lv_large_probe_summary.json`, generated by
`benchmarks/summarize_bmopf_combined_mv_lv_large_probe.jl`.

This demonstrates full-case model construction and solver startup/termination
at the previously guarded scale, but the iteration-limited points are not
convergence evidence and do not support policy ranking, allocator peak-memory,
or runtime-complexity claims. The next deliverable is a reviewed larger
iteration budget or allocator-level peak telemetry.

## 2026-08-27 reviewed full-case budget extension

The 56,142-variable combined MV/LV case was rerun for all three policies with
a reviewed 10-iteration and 30-second-per-policy budget. Every policy reached
Ipopt and returned one result, but all remained `ITERATION_LIMIT` infeasible
points. The normalized evidence is
`docs/bmopf_combined_mv_lv_budget_extension_summary.json`, generated by
`benchmarks/summarize_bmopf_combined_mv_lv_budget_extension.jl`.

This confirms stable bounded termination at a higher iteration budget while
showing that the current initialization does not converge within ten
iterations. It is not a policy ranking, allocator-memory measurement, or
runtime-complexity result. The next deliverable is a reviewed 50-iteration
full-case probe or allocator-level peak telemetry.

## 2026-08-27 reviewed 50-iteration full-case probe

The full 56,142-variable combined MV/LV case was rerun for all three policies
with a 50-iteration and 120-second-per-policy budget. Every policy reached
Ipopt and returned one result, but all remained `ITERATION_LIMIT` with
`INFEASIBLE_POINT` primal status. The campaign now explicitly applies all
56,142 completed start values before solving. The normalized evidence is
`docs/bmopf_combined_mv_lv_50iter_probe_summary.json`, generated by
`benchmarks/summarize_bmopf_combined_mv_lv_50iter_probe.jl`.
...
## 2026-08-27 feasibility-relaxed full-case initialization probe

BMOPFTools' public `solve_feasibility_opf` path was exercised on the same full
56,142-variable combined MV/LV source case. With a 50-iteration and 60-second
guard installed using correctly typed Ipopt attributes, the relaxed problem
returned `LOCALLY_SOLVED` in 3.1 seconds and exposed 53,236 finite,
topology-aware initialization scalars across 3,409 buses and 13,309 terminals.
The normalized evidence is
`docs/bmopf_combined_mv_lv_feasibility_probe_summary.json`, generated by
`benchmarks/bmopf_combined_mv_lv_feasibility_probe.jl`.

The result is a practical initialization success, but not a hard-feasible OPF:
the relaxed solve retains 354.31 A of aggregate KCL slack, so the zero-slack
gate is false. This also exposed an integration detail: Ipopt's integer
`max_iter` option must be applied through a model hook because passing it via
BMOPFTools' numeric `solver_options` converts it to a floating-point value.
...
## 2026-08-27 voltage-start API contract review

The current BMOPFTools `main` checkout (revision `1d9954ca`) exposes the
staged build, model, bases, KCL, and native-start entry points needed to build
the experiment, but it does not expose a public read/write voltage-start
transfer seam. The audit in `docs/bmopf_voltage_start_api_summary.json`,
generated by `benchmarks/audit_bmopf_voltage_start_api.jl`, records the two
missing proposal symbols:
`opf_voltage_start_values(ctx; units=:SI)` and
`set_opf_voltage_start_values!(ctx, starts; units=:SI)`.

The proposed contract specifies physical-vs-working coordinate semantics,
per-bus base conversion, grounded-terminal handling, finite/key validation,
lifecycle placement, and applied/skipped counts. Until BMOPFTools reviews and
implements that additive interface, the transfer benchmark's `ctx.vars`
access remains explicitly experimental. This closes the API review deliverable
...
## 2026-08-27 API ownership tranche 4

The persisted API ownership review advanced another deterministic 24-name
batch. The ledger now carries forward 164 decisions and records 188 reviewed
root-only names: 119 retain root compatibility and 69 remain Advanced
candidates. This tranche contains 24 additional root-compatibility decisions;
no exports, aliases, or type identities were changed, and automatic migration
remains disabled.
...
## 2026-08-27 API ownership tranche 8

The persisted API ownership review advanced another deterministic 24-name
batch. It now carries forward 260 decisions and records 284 reviewed root-only
names: 215 retain root compatibility and 69 remain Advanced candidates. This
...
## 2026-08-27 API ownership tranche 7

The persisted API ownership review advanced another deterministic 24-name
batch. It now carries forward 236 decisions and records 260 reviewed root-only
names: 191 retain root compatibility and 69 remain Advanced candidates. This
tranche adds 24 root-compatibility decisions; exports, aliases, and type
identities are unchanged, and automatic migration remains disabled.
...
## 2026-08-27 API ownership tranche 6

The persisted API ownership review advanced another deterministic 24-name
batch. It now carries forward 212 decisions and records 236 reviewed root-only
names: 167 retain root compatibility and 69 remain Advanced candidates. This
tranche adds 24 root-compatibility decisions; exports, aliases, and type
identities are unchanged, and automatic migration remains disabled.
...
## 2026-08-27 API ownership tranche 9

The persisted API ownership review advanced another deterministic 24-name
batch. It now carries forward 284 decisions and records 308 reviewed root-only
names: 239 retain root compatibility and 69 remain Advanced candidates. This
tranche adds 24 root-compatibility decisions; exports, aliases, and type
identities are unchanged, and automatic migration remains disabled.
...
## 2026-08-27 API ownership tranche 10

The persisted API ownership review advanced another deterministic 24-name
batch. It now carries forward 308 decisions and records 332 reviewed root-only
names: 245 retain root compatibility and 87 are Advanced candidates. This
tranche adds 6 root-compatibility decisions and 18 Advanced-candidate
...
## 2026-08-27 API ownership tranche 11

The persisted API ownership review advanced another deterministic 24-name
batch. It now carries forward 332 decisions and records 356 reviewed root-only
names: 265 retain root compatibility and 91 are Advanced candidates. This
tranche adds 20 root-compatibility decisions and 4 Advanced-candidate
decisions; exports, aliases, and type identities are unchanged, and automatic
migration remains disabled.
...
## 2026-08-27 API ownership tranche 12

The persisted API ownership review advanced another deterministic 24-name
batch. It now carries forward 356 decisions and records 380 reviewed root-only
names: 289 retain root compatibility and 91 are Advanced candidates. This
tranche adds 24 root-compatibility decisions; exports, aliases, and type
identities are unchanged, and automatic migration remains disabled.
...
## 2026-08-27 API ownership tranche 13

The persisted API ownership review advanced another deterministic 24-name
batch. It now carries forward 380 decisions and records 404 reviewed root-only
names: 313 retain root compatibility and 91 are Advanced candidates. This
tranche adds 24 root-compatibility decisions; exports, aliases, and type
identities are unchanged, and automatic migration remains disabled.
...
## 2026-08-27 API ownership tranche 14

The persisted API ownership review advanced another deterministic 24-name
batch. It now carries forward 404 decisions and records 428 reviewed root-only
names: 330 retain root compatibility and 98 are Advanced candidates. This
tranche adds 17 root-compatibility decisions and 7 Advanced-candidate
decisions; exports, aliases, and type identities are unchanged, and automatic
migration remains disabled.
...
## 2026-08-27 API ownership tranche 15

The persisted API ownership review advanced another deterministic 24-name
batch. It now carries forward 428 decisions and records 452 reviewed root-only
names: 353 retain root compatibility and 99 are Advanced candidates. This
tranche adds 23 root-compatibility decisions and 1 Advanced-candidate
decision; exports, aliases, and type identities are unchanged, and automatic
migration remains disabled.
...
## 2026-08-27 API ownership tranche 16

The persisted API ownership review advanced another deterministic 24-name
batch. It now carries forward 452 decisions and records 476 reviewed root-only
names: 377 retain root compatibility and 99 are Advanced candidates. This
tranche adds 24 root-compatibility decisions; exports, aliases, and type
identities are unchanged, and automatic migration remains disabled.
...
## 2026-08-27 API ownership tranche 17

The persisted API ownership review advanced another deterministic 24-name
batch. It now carries forward 476 decisions and records 500 reviewed root-only
names: 401 retain root compatibility and 99 are Advanced candidates. This
tranche adds 24 root-compatibility decisions; exports, aliases, and type
identities are unchanged, and automatic migration remains disabled.
