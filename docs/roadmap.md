...
`benchmarks/phase_only_controller_ipopt_campaign.jl` to `benchmarks/common.jl`,
bringing the shared helper to fifty-five core runners while preserving the
matched four-record controller-rich campaign schema. Its local smoke run
...
increment migrated `benchmarks/phase_only_nonlinear_ipopt_campaign.jl`; its
