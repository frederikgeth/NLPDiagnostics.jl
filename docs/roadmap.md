...
`benchmarks/summarize_bmopf_smoke.jl` to `benchmarks/common.jl`, bringing the
shared helper to thirty-six core runners while preserving the smoke-case,
integrity, profile, and saved-result summary schema. Its local smoke run
completed successfully against the merged smoke-results artifact, retaining
both error cases and source-schema warning counts explicitly. The
...
thirty-six core release, rank, runtime, and audit runners; the BMOPF campaign and
