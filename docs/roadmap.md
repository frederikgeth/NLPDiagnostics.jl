...
`benchmarks/real_99bus_phase_only_tolerance_matrix.jl` to
`benchmarks/common.jl`, bringing the shared helper to sixty-four core runners
while preserving the four-case tolerance/budget sensitivity schema. Its
bounded local smoke selected the baseline tolerance case and produced a report
with all six phase-only runs locally solved, six solver-floor-calibrated
