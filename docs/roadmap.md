...
`benchmarks/summarize_bmopf_sparse_corpus.jl` to `benchmarks/common.jl`,
bringing the shared helper to thirty-seven core runners while preserving the
sparse-first readiness and resource-guard summary schema. Its local smoke run
completed successfully against a two-row sparse-corpus fixture, retaining
profile-complete and resource-skipped phases and their readiness boundaries.
The
...
thirty-seven core release, rank, runtime, and audit runners; the BMOPF campaign and
