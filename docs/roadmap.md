...
`benchmarks/check_benchmark_environment.jl` to `benchmarks/common.jl`, bringing
the shared helper to eighty-eight core runners and adding a shared JSON-string
serialization path for stdout-only preflight artifacts. The known environment
check passed with all required packages, all optional integrations, and the
PowerIO C ABI available. The prior increment migrated
