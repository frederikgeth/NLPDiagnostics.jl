...
`INFEASIBLE_POINT` primal status. The normalized evidence is
...
initialization remains non-convergent even with 50 iterations, while the
process remains stable and bounded. The next deliverable is a convergent
full-case initialization investigation together with allocator-level peak
telemetry; further iteration increases alone are not yet justified.
